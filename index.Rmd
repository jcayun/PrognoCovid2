...
```{r echo=FALSE, comment=NA, results='hide',message=FALSE, warning = FALSE}
library(readr)
COVIDRepositorioCRF_DATA <- read_csv("COVIDRepositorioCRF_DATA_2021-10-11_2247.csv")
...
```{r echo=FALSE, comment=NA,message=FALSE, warning = FALSE}
table1(~  data$sexo + data$hospital + data$edad_participante + data$PCR + data$prueba_serologica + data$sospecha_clinica + data$tabaco + data$alcohol + data$drogas + data$dias_hosp + data$num_enf_previas, data=data, overall = "Total")
...
data3 <- data3[data3$hospital == 1,]
data3 <- data3[!(is.na(data3$cod_participante)),]

data4 <- COVIDRepositorioCRF_DATA
data4 <- data4[data4$redcap_data_access_group == 'hap',]
...
```{r echo=FALSE, comment=NA,message=FALSE, warning = FALSE}
table1(~  data3$sexo + data3$edad_participante + data3$PCR + data3$prueba_serologica + data3$sospecha_clinica + data3$tabaco + data3$alcohol + data3$drogas + data3$dias_hosp + data3$num_enf_previas, data=data3, overall = "Total")
```
...
```{r echo=FALSE, comment=NA,message=FALSE, warning = FALSE}
library(readxl)
Hospital_COVID_1_ <- read_excel("basescovid/Hospital-COVID (1).xlsx")

data10 <- Hospital_COVID_1_

```

```{r echo=FALSE, comment=NA,message=FALSE, warning = FALSE}

data10$cod_universal <- data10$ID
data10$sexo <- data10$Gender...4
data10$edad_participante <- data10$Age
data10$tabaco <- data10$Smoker
data10$alcohol <- data10$Alcohol
data10$dias_hosp <- data10$`Dias Hosp`
data10$fiebre <- data10$Fiebre
data10$tos_seca <- data10$Tos
data10$tos_productiva <- data10$Tos
data10$dolor_pecho <- data10$`Dolor Torax`
data10$disnea <- data10$Disnea
data10$diarrea <- data10$Diarrea
data10$dolor_abdominal <- data10$`Dolor Abdominal`
data10$mialgia <- data10$Mialgia
data10$odinofagia <- data10$Odinofagia
data10$adeusia <- data10$Augeusia
data10$cefalea <- data10$Cefalea
data10$anosmia <- data10$Anosmia

data10$hipertension  <- data10$HTA 
data10$ins_renal_cronica   <- data10$CKD
data10$epoc   <- data10$COPD
data10$diabetes2 <- data10$T2DM  
data10$cancer  <- data10$Cancer

data10$peso <- data10$Weight
data10$altura <- data10$Height
data10$imc <- data10$BMI
data10$total_uci  <- data10$`Dias en UCI`
data10$nesecidad_vmi <- data10$VMI 


data10_extraido <- data.frame(     
...
#Generación de una base de datos común
...
#Resultados de sintomas al ingreso
...
#Resultados de enfermedades previas
...
#Resultados de evaluación al egreso
